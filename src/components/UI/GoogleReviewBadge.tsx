...
const MOCK_REVIEWS = [
    ...
    { name: "David C.", rating: 5, text: "Great fusion of flavors. The best Indo-Chinese in London.", date: "1 week ago" },
    { name: "Emily R.", rating: 4, text: "Lovely atmosphere and friendly staff. Highly recommend the lamb chops.", date: "3 weeks ago" }
];

// Helper to render stars safely
const RenderStars = ({ size = 14, rating = 4.5 }: { size?: number, rating?: number }) => {
    return (
                        ...
                        {isHalf && (
                            <div style={{
                                ...
                                <Star
                                    ...
                                    fill="#FBBC05"
                                    stroke="none"
                                    style={{ position: 'absolute', top: 0, left: 0, minWidth: size }}
                                />
                            </div>
                        )}
                    </div>
            ...
            >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontWeight: '600', color: '#202124', fontSize: '15px', lineHeight: '1' }}>4.5</span>
                        ...
                        >
                            ...
                            <div style={{ padding: '20px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    <span style={{ fontSize: '24px', fontWeight: 'bold', color: '#202124' }}>4.5</span>
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <RenderStars size={16} rating={4.5} />
                                        <span style={{ fontSize: '12px', color: '#5f6368', marginTop: '2px' }}>based on 512 reviews</span>
                                    </div>
                                </div>
                                <button
                            ...
                            <div style={{ maxHeight: '300px', overflowY: 'auto', padding: '0' }}>
                                {MOCK_REVIEWS.map((review, idx) => (
                                    <div key={idx} style={{ padding: '16px 20px', borderBottom: '1px solid #f0f0f0' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                            <span style={{ fontWeight: '600', fontSize: '14px', color: '#202124' }}>{review.name}</span>
                                            <span style={{ fontSize: '12px', color: '#5f6368' }}>{review.date}</span>
                                        </div>
                                        <div style={{ display: 'flex', marginBottom: '8px' }}>
                                            <RenderStars size={12} rating={review.rating} />
                                        </div>
                                        <p style={{ fontSize: '13px', color: '#3c4043', margin: 0, lineHeight: '1.5' }}>
                                            {review.text}
                                        </p>
                                    </div>
                                ))}
                            ...
                            {/* Footer Action */}
                            <div style={{ padding: '16px 20px', backgroundColor: '#f8f9fa', textAlign: 'center' }}>
                                <a
