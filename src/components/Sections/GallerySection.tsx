...
const GallerySection = ({ location }: { location?: string }) => {
  ...
  const isDarkTheme = isSoho || isCovent;
  ...
  return (
    <section
      ...
      style={{
        padding: "4rem 1rem",
        backgroundColor: isDarkTheme ? "#151313" : "#F4F4F2",
        backgroundImage: isSoho
          ? `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${sohoTexture})`
          : isCovent
            ? `linear-gradient(rgba(0,0,0,0.3), rgba(0,0,0,0.3)), url(${coventTexture})`
            : `url(${bgTexture})`,
        backgroundBlendMode: isDarkTheme ? "normal" : "multiply",
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundAttachment: isDarkTheme ? "fixed" : "scroll", // Keep clean look for light paper, fixed for dark texture
        overflow: "hidden",
        minHeight: "150vh",
      }}
    >
      ...
      {images.map((src, i) => (
        <motion.div
          ...
          whileHover={{
            ...
            boxShadow: "0 0 40px rgba(255, 255, 255, 0.4)", // Stronger glow on hover
          }}
          style={{
            position: "relative",
            borderRadius: "0px", // Pointy edges
            overflow: "hidden",
            // Glow Effect: Background light
            boxShadow: "0 0 20px rgba(255, 255, 255, 0.2)",
            cursor: "pointer",
            background: "rgba(255,255,255,0.1)", // Subtle tint behind
          }}
        >
          <img
            src={src}
